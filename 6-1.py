def shift_up(index, heap):#функция поднятия вверх элемента
    while heap[index] > heap[(index-1)//2] and (index-1)//2 >= 0:#если элемет больше, то меняем местами
        heap[index], heap[(index-1)//2] = heap[(index-1)//2], heap[index]
        index = (index-1)//2
    return index+1#возвращаем индекс+1

def shift_down(index, heap):#опускаем элемент вниз
...
def main():
    ...
    for i in result:# для каэдого элемта в результате, если список то вывести без знаков, если переменнаято обычный вывод
        if type(i) == tuple:
            print(*i)
        else:
            print(i)
