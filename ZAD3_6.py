class Heap():#создаем класс
    def __init__(self, arr):#инициализация 
        self.__heap = arr
    ...
    def shift_down(self, index):
        while 2*index+1 < self.length:
            ...
            index = child_index#функция перемещает массив вверх,пока он снава не станет кучей и добавляется индекс 

    def extract(self):
        self.__heap[0], self.__heap[self.length-1] = self.__heap[self.length-1], self.__heap[0]
        value=self.__heap.pop()
        self.shift_down(0)
        return value#нахождение и извлечение максимального элемента 
